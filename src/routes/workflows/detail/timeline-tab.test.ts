...
describe('TimelineTab', () => {
  test('shows an empty state with no timeline entries', async () => {
    ...
    const liveObservations = new WorkflowLiveObservations(new InertFleet(), inertQueryClient(), 'wf-1');
  ...
  test('clicking a step selects it and shows the linked-selection chip, Clear removes it', async () => {
    ...
    const { getByText, queryByText } = render(TimelineTabHarness, {
      props: { client: baseClient(entries), workflow: workflow({ id: 'wf-select-1' }), liveObservations },
    });
  ...
  test('the Failed quick filter narrows the rendered steps', async () => {
    ...
    const { getByText, getByRole, queryByText } = render(TimelineTabHarness, {
      props: { client: baseClient(entries), workflow: workflow({ id: 'wf-filter-1' }), liveObservations },
    });
  ...
  test('shows the Finalizing badge when the live observations report a still-in-flight finalizer', async () => {
    ...
    fleet.emit({ kind: 'workflow:cancelled', workflowId: 'wf-1', sequence: 1, cursor: '1', emittedAtMs: 1, payload: {} });

    const { getByText } = render(TimelineTabHarness, {
      props: { client: baseClient([]), workflow: workflow({ status: 'cancelled' }), liveObservations },
    });
  ...
  test('an unambiguous pending async activity badges the matching step', async () => {
    ...
    fleet.emit({
      ...
      cursor: '1',
      emittedAtMs: 1,
      payload: { token: 'tok-1', operationId: 'op-1', activityName: 'printShippingLabel', attempt: 1 },
    });
    const entries = [entry({ step: 1, operationType: 'activity', operationLabel: 'printShippingLabel', status: 'running' })];

    const { getByText } = render(TimelineTabHarness, {
      props: { client: baseClient(entries), workflow: workflow({ id: 'wf-1', status: 'running' }), liveObservations },
    });
