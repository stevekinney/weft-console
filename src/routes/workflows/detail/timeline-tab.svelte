<script lang="ts">
  ...
  const pageEntries = $derived(paginated ? timelineEntriesForPage(filteredEntries, pageIndex) : filteredEntries);
...
<div class="weft-timeline-tab">
  <FinalizerStrip finalizingLive={liveObservations.finalizingLive} teardown={liveObservations.finalizerTeardown} />
  ...
  {:else}
    <div class="weft-timeline-tab__toolbar">
      <SegmentedControl id={`timeline-quick-filter-${workflow.id}`} label="Quick filter" hideLabel density="toolbar" value={quickFilter} onchange={(next) => (quickFilter = next)}>
        ...
        <RunStepTimeline {steps} label={`${workflow.type} timeline`}>
          {#snippet children(step)}
            {#if attachedPendingActivities.some((activity) => activity.stepId === step.id)}
              {@const activity = attachedPendingActivities.find((candidate) => candidate.stepId === step.id)}
              {#if activity}
  ...
  {#if selection.selectedStepId !== null}
    <div class="weft-timeline-tab__selection-chip">
      ...
      <button type="button" class="weft-timeline-tab__selection-clear" onclick={clearTimelineSelection}>
